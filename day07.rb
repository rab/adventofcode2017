...
class Program
  class << self
    ...
    def root
      candidate = tower.values.first.name
      candidate = tower[candidate].name while tower[candidate]
      candidate
    end
  end

  def programs ; self.class.programs ; end
  def tower    ; self.class.tower    ; end

  attr_accessor :name, :weight, :holding

  def initialize(name, weight, *holding)
    @name = name
    @weight = weight
    @holding = holding || []
    programs[name] = self
    self.holding.each do |held|
      self.tower[held] = self
    end
  end

  def total_weight
    @total_weight ||= holding.reduce(self.weight) {|total, held| total + programs[held].total_weight }
  end

  def balanced?
    return true if holding.empty?
    target = programs[holding.first].total_weight
    holding.all? {|held| programs[held].total_weight == target }
  end

  def wrong?
    return false if holding.empty?
    ! balanced? && holding.all? {|held| programs[held].balanced?}
  end

  def to_s
    str = "#{name} (#{weight})"
    unless holding.empty?
      str << " -> "; sep = ''
      holding.each do |held|
        str << sep; sep = ', '
        str << "#{held}[#{programs[held].total_weight}]"
      end
    end
    str
  end
...
input.each_line do |line|
  # puts line
  name, weight, all_holding = /\A(\w+) \((\d+)\)(?: -> ([a-z, ]*))?$/.match(line).captures
  holding = all_holding&.split(/, /) || []
  Program.new(name, weight.to_i, *holding)
end

puts "Part1:", Program.root
...
holding_bad = Program.programs.values.select(&:wrong?).first
weights = holding_bad.holding.count_by{|held| Program.programs[held].total_weight}
bad_weight = weights.key(1)
good_weight = (weights.keys - [bad_weight]).first
delta = good_weight - bad_weight
bad_name = holding_bad.holding.detect {|held| Program.programs[held].total_weight == bad_weight }
bad_program = Program.programs[bad_name]

require 'pp'
pp '', holding_bad, bad_weight, good_weight.inspect, bad_name, bad_program.to_s

puts "Part2:", bad_program.weight + delta
