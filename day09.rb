...
class Lexer
  ...
  def total
    total = 0
    trash = 0
    while (char = @input[@position])
      if char == '!'
        @position += 1
        puts "skips !#{@input[@position]}" if $DEBUG
      elsif @garbage
        if char == '>'
          @garbage = false
          puts "> ends garbage" if $DEBUG
        else
          trash += 1
          puts "#{char} is garbage" if $DEBUG
        end
      else
        if char == '<'
          @garbage = true
          puts "< starts garbage" if $DEBUG
        elsif char == '}'
          @score -= 1
          puts "} closes group" if $DEBUG
        elsif char == '{'
          total += @score
          puts "{ starts group for #{@score}" if $DEBUG
          @score += 1
        end
      end
      @position += 1
...
if $DEBUG
  [
    ...
    ['{{<!!>},{<!!>},{<!!>},{<!!>}}', 9],
    ['{{<a!>},{<a!>},{<a!>},{<ab>}}', 3],
  ].each do |stream, expected|
    print "#{stream} expects #{expected}"
    if (actual = Lexer.new(stream).total[:score]) == expected
      puts " √"
    else
      puts ", but got #{actual}"
    end
...
score, trash = Lexer.new(input).total.values_at(:score, :trash)
puts "Part1:", score
puts "Part2:", trash
