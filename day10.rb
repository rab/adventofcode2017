...
# current position.  Move the current position forward by that length plus the skip size.
...
# this sublist (3 0 1 2 4 into 4 2 1 0 3) produces: 3) ([4] 2 1 0.  Finally, the current position
# moves forward by 8: 3 4 2 1 [0]. The skip size increases to 4.
#
...
lengths = input.scan(/\d+/).map(&:to_i)

list = (0..255).to_a

def twist(list, lengths)
  size = list.size
  pos = 0
  skip = 0

  lengths.each do |length|
    print "\n[#{pos},#{length},#{skip}]: #{list.inspect} => "
    ...
    # else                          # worry about wrap
    #   a = size - pos
    #   z = length - a
    #   sublist = (list[pos, a] + list[0, z]).reverse
    #   list[pos, a] = sublist.slice!(0, a)
    #   list[0, z] = sublist
    # end
    print list.inspect
...
list = twist list, lengths
check = list[0,2].reduce(:*)
...
# bytes instead. Unless otherwise specified, convert characters to bytes using their ASCII
# codes. This will allow you to handle arbitrary ASCII strings, and it also ensures that your
# input lengths are never larger than 255. For example, if you are given 1,2,3, you should convert
...
codes = input.strip.each_char.map(&:ord).concat seed
...
sparse_hash = byte_twist((0..255).to_a, codes)

dense_hash = sparse_hash.each_slice(16).map{|sl| sl.reduce(:^)}

puts "Part2:", dense_hash.map {|h| "%02x"%h }.join
